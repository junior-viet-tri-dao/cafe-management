...
//    @Query(value = "SELECT " +
//     "d.Ngay, " +
//     "ISNULL((SELECT SUM(tong_tien) FROM dbo.hoa_don WHERE trang_thai = 1 AND CAST(ngay_gio_tao AS DATE) = d.Ngay), 0) AS Thu, " +
//     "( " +
//     "   ISNULL((SELECT SUM(tong_tien) FROM dbo.don_nhap WHERE ngay_nhap = d.Ngay), 0) + " +
//     "   ISNULL((SELECT SUM(so_tien) FROM dbo.chi_tieu WHERE ngay_chi = d.Ngay), 0) + " +
//     "   ISNULL((SELECT SUM(so_luong * don_gia_mua) FROM dbo.thiet_bi WHERE ngay_nhap = d.Ngay), 0) " +
//     ") AS Chi " +
//     "FROM ( " +
...
Page<Object[]> getDoanhThuChiTieuTheoNgay(@Param("fromDate") Date fromDate,
