...
public interface HoaDonRepository extends JpaRepository<HoaDon, Integer> {
  ...
  "      AND CAST(hd.ngay_gio_tao AS DATE) = d.Ngay" +
  "  ), 0) AS Thu, " +
  "  ( " +
  "    ISNULL(( " +
  "      SELECT SUM(tong_tien) " +
  "      FROM dbo.don_nhap dn " +
  "      WHERE dn.ngay_nhap = d.Ngay" +
  "    ), 0) " +
  "    + " +
  "    ISNULL(( " +
  "      SELECT SUM(so_tien) " +
  "      FROM dbo.chi_tieu ct " +
  "      WHERE ct.ngay_chi = d.Ngay" +
  "    ), 0) " +
  "    + " +
  "    ISNULL(( " +
  "      SELECT SUM(so_luong * don_gia_mua) " +
  "      FROM dbo.thiet_bi tb " +
  "      WHERE tb.ngay_nhap = d.Ngay" +
  "    ), 0) " +
  "  ) AS Chi " +
  "FROM ( " +
