...
public Object danhSachBanRanh(@RequestParam Integer maBanCu) {
    ...
    public Object layThucDonTheoBan(@PathVariable Integer maBan) {
        ...
        if (chiTiet == null || chiTiet.getHoaDon() == null) {
            return java.util.Map.of("success", false, "message", "Không tìm thấy hóa đơn cho bàn này.");
        }
...
//         List<Map<String, Object>> thucDonData = new ArrayList<>();
// for (ThucDon td : thucDonList) {
...
//     thucDonData.add(map);
// }
// bạn đang dùng Map.of(...) (Java 9+) và Java compiler không thể tự động suy luận kiểu generic chính xác khi dùng với List<Map<String, Object>>.
    //    Cách 1: Dùng new HashMap thay cho Map.of (tương thích mọi Java version, không lỗi generic): 
List<Map<String, Object>> thucDonData = thucDonList.stream().map(td -> {
    ...
    @ResponseBody
    public Object goiMon(@RequestParam Integer maHoaDon, @RequestParam java.util.List<Integer> maThucDon, @RequestParam java.util.List<Integer> soLuong) {
        if (maThucDon.size() != soLuong.size()) {
            return java.util.Map.of("success", false, "message", "Dữ liệu gửi lên không hợp lệ.");
        }
        HoaDon hoaDon = hoaDonRepository.findById(maHoaDon).orElse(null);
        if (hoaDon == null) {
            return java.util.Map.of("success", false, "message", "Không tìm thấy hóa đơn.");
        }
        double tongTien = 0;
        for (int i = 0; i < maThucDon.size(); i++) {
            Integer maTD = maThucDon.get(i);
            Integer sl = soLuong.get(i);
            com.vn.model.ThucDon td = thucDonRepository.findById(maTD).orElse(null);
            if (td == null) continue;
            double gia = td.getGiaTienHienTai();
            double thanhTien = gia * sl;
            tongTien += thanhTien;
            com.vn.model.ChiTietHoaDon cthd = new com.vn.model.ChiTietHoaDon();
        ...
        return java.util.Map.of("success", true, "message", "Gọi món thành công!", "tongTien", tongTien);
    }
