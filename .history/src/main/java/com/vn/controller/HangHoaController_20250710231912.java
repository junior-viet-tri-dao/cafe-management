...
public class HangHoaController {
    ...
    public String xuatHang(@Valid @ModelAttribute("xuatHang") DonXuat donXuat, Model model, RedirectAttributes redirectAttributes, BindingResult result) {
        ...
        if (donXuatRepository.existsByMaDonXuat(donXuat.getMaDonXuat())) { 
            result.rejectValue("maDonXuat", null, "Mã đơn xuất đã tồn tại");
        }
        ...
        if (donXuat.getSoLuong() > hangHoa.getSoLuong()) {
            ...
            List<HangHoa> hangHoaList = hangHoaRepository.findAll();
            model.addAttribute("hangHoaList", hangHoaList);
            if (donXuat.getHangHoa() != null && donXuat.getHangHoa().getMaHangHoa()!= null) {
                HangHoa hv = hangHoaRepository.findByMaHangHoa(donXuat.getHangHoa().getMaHangHoa());
                donXuat.setHangHoa(hv);
            }
            model.addAttribute("xuatHang", donXuat);
            return "admin/hanghoa/hanghoa-xuathang";
        }
        ...
        int soLuongHienTai = hangHoa.getSoLuong() - donXuat.getSoLuong();
        hangHoa.setSoLuong(soLuongHienTai);
        if(soLuongHienTai == 0){
            hangHoa.setDonGia(0.0);
        } 
    ...
    public String createHangHoa(@Valid @ModelAttribute("hangHoa") HangHoa hangHoa, BindingResult result,
        ...
        if(hangHoaRepository.existsByTenHangHoa(hangHoa.getTenHangHoa())) {
            ...
            if (result.hasErrors()) {
            if (hangHoa.getDonViTinh() != null && hangHoa.getDonViTinh().getMaDonViTinh() != null) {
                DonViTinh dvt = donViTinhRepository.findByMaDonViTinh(hangHoa.getDonViTinh().getMaDonViTinh());
                hangHoa.setDonViTinh(dvt);
            }
