...
public class ThucDonController {
    ...
    public String createTable(@Valid @ModelAttribute("thucDon") ThucDon thucDon, BindingResult result, Model model, RedirectAttributes redirectAttributes) {
    ...
    public String updateThucDon(@Valid @ModelAttribute("thucDon") ThucDon thucDon, BindingResult result,
      ...
      public String deleteThucDon(@RequestParam("Ids") List<Integer> Ids,  RedirectAttributes redirectAttributes) {
        List<ThucDon> thucDons = thucDonRepository.findAllById(Ids);
        for (ThucDon thucDon : thucDons) {
            thucDon.setIsDeleted(true);
            thucDonRepository.save(thucDon);
        }
        redirectAttributes.addFlashAttribute("successMessage", "Đã xóa thành công các món ăn đã chọn.");
        return "redirect:/admin/thucdon/thucdon-list";
    }
}
