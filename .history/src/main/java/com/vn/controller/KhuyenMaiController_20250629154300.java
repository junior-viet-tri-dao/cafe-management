...
public class KhuyenMaiController {
    ...
    public String listTable(@RequestParam(defaultValue = "0") int page,
                            ...
                            Model model) {
        Page<KhuyenMai> posts;
        if (keyword != null && !keyword.isEmpty()) {
            posts = khuyenMaiRepository.findAll(PageRequest.of(page, size)); // TODO: thay bằng search nếu có
        } else {
            posts = khuyenMaiRepository.findAll(PageRequest.of(page, size));
        }
    ...
    public String createTable(@Valid @ModelAttribute("khuyenMai") KhuyenMai khuyenMai, BindingResult result, Model model) {
      ...
      @GetMapping("/admin/khuyenmai/thucdon-update/{id}")
