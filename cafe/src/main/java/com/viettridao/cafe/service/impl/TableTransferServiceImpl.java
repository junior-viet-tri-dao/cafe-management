...
public class TableTransferServiceImpl implements TableTransferService {
    ...
    public void transferTable(Integer fromTableId, Integer toTableId) {
        ...
        // ❌ Kiểm tra các trường hợp chuyển không hợp lệ
        if ((fromStatus == TableStatus.AVAILABLE && (toStatus == TableStatus.RESERVED || toStatus == TableStatus.OCCUPIED)) ||
        ...
        // ✅ Lấy hóa đơn chưa thanh toán ở bàn nguồn
        InvoiceEntity invoice = invoiceRepository
                .findTopByReservations_Table_IdAndStatusAndIsDeletedFalseOrderByCreatedAtDesc(
                        fromTableId, InvoiceStatus.UNPAID);

        if (invoice == null) {
            throw new RuntimeException("Không có hóa đơn chưa thanh toán ở bàn nguồn.");
        }

        // ❌ Kiểm tra bàn đích không có hóa đơn chưa thanh toán
        InvoiceEntity targetInvoice = invoiceRepository
                .findTopByReservations_Table_IdAndStatusAndIsDeletedFalseOrderByCreatedAtDesc(
                        toTableId, InvoiceStatus.UNPAID);

        if (targetInvoice != null) {
            throw new RuntimeException("Bàn đích đang có hóa đơn chưa thanh toán.");
        }

        // ✅ Xóa reservation cũ và tạo reservation mới với bàn đích
        List<ReservationEntity> oldReservations = reservationRepository.findByInvoice_IdAndIsDeletedFalse(invoice.getId());
        ...
        // ✅ Cập nhật trạng thái bàn sau khi chuyển
